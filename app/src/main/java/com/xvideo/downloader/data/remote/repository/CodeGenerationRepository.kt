...
class CodeGenerationRepository(private val context: Context) {
    ...
    companion object {
        ...
        fun getInstance(context: Context): CodeGenerationRepository {
            return instance ?: synchronized(this) {
                instance ?: CodeGenerationRepository(context).also { instance = it }
            }
        }
    }
    ...
    private fun extractCodeBlocks(response: String): List<CodeBlock> {
        ...
        regex.findAll(response).forEach { match ->
            ...
            val code = if (filePath != null) {
                // Remove the file path comment line
                rawCode.lines().dropWhile { it.trim().startsWith("//") || it.trim().startsWith("#") && it.contains("文件路径") }
                    .joinToString("\n").trim()
            } else {
                rawCode
            }
    ...
    private fun detectProjectName(blocks: List<CodeBlock>): String? {
        for (block in blocks) {
            ...
            // Use the top-level directory from the first file path
    ...
    fun saveFile(filePath: String, content: String): Boolean {
        return try {
            ...
            Log.d(TAG, "Auto-saved file: $filePath")
            true
        } catch (e: Exception) {
